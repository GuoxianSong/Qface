...
void Mesh3D::UpdateMesh(void)
{
	if (!isValid())
	{
		////when read in a vertex set,no information of faces,added by D.D
...
{
	for (VERTEX_ITER viter = pvertices_list_->begin(); viter!=pvertices_list_->end(); viter++)	//modify
	{
...
void Mesh3D::DeUnify(float size,std::vector<Vec3f> &mesh_point)
{
	mesh_point.resize(pvertices_list_->size());
	...
	for (size_t i = 0; i != pvertices_list_->size(); i++)
	{
		p = pvertices_list_->at(i)->position_;
		p[1] = 0-p[1];
		mesh_point.at(i) = p / scaleV + centerPos;
	}
}
